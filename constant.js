        ...
        const skillDescriptions = {
            ...
            "Применение классических и трендовых методик / методологий в найме специалистов (глубокий анализ кандидатов по соцсетям и цифровому следу, применение кейс-интервью, стресс-интервью, использование STAR, PARLA, SPIN-метод)": {
                1: "Осведомлён о базовых техниках (STAR, кейсы), может задать типичные вопросы.",
                2: "Периодически использует комбинированные подходы (лёгкий стресс, кейсы), но следует готовым методичкам без глубокой адаптации.",
                3: "Системно применяет разные интервью-методики (поведенческие, ситуативные), подбирает их под уровень вакансии, собирает минимум данных из соцсетей.",
                4: "Анализирует цифровой след в соцсетях (LinkedIn, GitHub), использует различные форматы интервью (глубокий стресс, кейсы + STAR), адекватно комбинирует их.",
                5: "Создаёт внутренние материалы (гайд по методикам), обосновывает выбор техники под конкретную роль (IT, финансы, продажи и т.д.).",
                6: "Оценивает эффективность разных методик, внедряет многотомные структуры интервью (несколько этапов, ассессменты), формирует статистику и оптимизирует процесс.",
                7: "Интегрирует трендовые инструменты (AI-оценка, психометрические тесты во время интервью), обучает все HR-подразделения этим методикам.",
                8: "Постоянно обновляет методологии в соответствии с глобальными трендами рынка, выступает с докладами в профсообществах, внедряет собственные авторские подходы внутри компании.",
                9: "Формирует уникальные отраслевые методологии, которые высоко оцениваются внешними экспертами, помогает развивать рынок рекрутинга инновационными практиками."
            },

            "Менеджмент. Управление вакансиями, процессами, временем и командой (планирование, расстановка приоритетов, принятие решений, ненасильственный менеджмент 3.0)": {
                1: "Одновременно ведёт 1-2 вакансии, фиксирует сроки закрытия и старается их соблюдать, обращается за помощью к старшему коллеге в сложных случаях.",
                2: "Планирует основные задачи (создание вакансии, интервью, согласование оффера) и отслеживает сроки, при необходимости вовремя эскалирует проблемы.",
                3: "Руководит несколькими вакансиями одновременно, расставляет приоритеты, фиксирует метрики (time-to-hire, cost-per-hire), своевременно докладывает о рисках.",
                4: "Использует доски, чек-листы, трекинг-системы для планирования; привлекает коллег из смежных отделов по мере надобности, делегирует задачи младшим рекрутерам.",
                5: "Оптимизирует процесс (сокращает «пробеги» между этапами), анализирует слабые места (частые отказы, долгие согласования), улучшает сроки найма.",
                6: "Активно внедряет гибкие методологии (Scrum, Kanban) в управлении рекрутингами, формирует высокую прозрачность по этапам, делится отчётами с командой.",
                7: "Применяет менеджмент 3.0: делегирование полномочий, поддерживающую обратную связь, мотивирует команду через вовлечение и общие ценности.",
                8: "Управляет крупными рекрутинговыми инициативами (массовые наймы, открытие новых филиалов), выстраивает эффективную структуру команды, оптимизирует набор KPI.",
                9: "Разрабатывает стратегию для комплексных HR-программ (перезапуск процессов найма, глобальные проекты), управляет мультифункциональными командами, формирует культуру менеджмента на уровне всей организации."
            },

            "Навыки продаж и сервиса, мотивация кандидатов, выявление их потребности, консультирование, умение продавать кандидатов нанимающему менеджеру": {
                1: "Озвучивает базовые плюсы вакансии и компании (стабильность, соцпакет), пытается понять ожидания кандидата.",
                2: "Расспрашивает о проблемах/пожеланиях кандидата, сопоставляет их с возможностями компании (ЗП, график, соцпакет), упоминает наиболее значимые выгоды.",
                3: "Глубже анализирует личные мотивы кандидата, аргументированно презентует ценности компании менеджеру, формирует «преимущества» на основе профиля.",
                4: "Применяет техники продаж (SPIN, выявление KPI у кандидата), умеет убеждать кандидата и менеджера в выгодности сотрудничества.",
                5: "Подстраивает «продающий» сценарий под конкретного кандидата (его личные драйверы и ограничения), разрабатывает индивидуальную мотивацию.",
                6: "Обучает команду рекрутеров навыкам переговоров и убеждения, проводит разбор сложных кейсов (когда кандидат «колеблется», менеджер «не убеждён»).",
                7: "Выстраивает «сервисную» культуру: стандарты коммуникации, системное понимание потребностей кандидатов, скорость ответа, формирует позитивный HR-бренд.",
                8: "Создаёт репутацию HR как полноценного партнёра бизнеса: управляет стратегиями удержания, глубинным анализом рынка компенсаций, способствует лояльности сотрудников.",
                9: "Развивает передовые продажи и сервис в HR: участвует в формировании рыночных стандартов, внедряет новейшие переговорные техники, выступает экспертом в профсообществах."
            },
            "Ведение и формирование аналитических данных, в том числе предиктивных, создание дашбордов и отчетов": {
                1: "Ведёт учёт кандидатов в Excel, умеет составлять базовый отчёт (число откликов, приглашений, отказов).",
                2: "Применяет фильтры и простые формулы, строит диаграммы (графики) для визуализации прогресса (воронки).",
                3: "Делает сводные таблицы, рассчитывает ключевые метрики: время закрытия, стоимость найма, разбивает статистику по источникам.",
                4: "Настраивает регулярные отчёты для руководства, создаёт понятные дашборды (Google Data Studio, Power BI), анализирует тенденции (рост/спад).",
                5: "Определяет причины изменений (почему растёт cost-per-hire или проседает конверсия), предлагает конкретные улучшения (смена источников, корректировка процесса).",
                6: "Применяет предиктивные модели (прогноз сроков найма, риск оттока), автоматизирует сбор данных через HR-системы и интеграции.",
                7: "Разрабатывает комплексные BI-решения (объединяет данные рекрутинга, HR-аналитику, финансы), обучает команду использовать метрики для принятия решений.",
                8: "Встраивает аналитику и прогностические индексы в управленческие дашборды всего банка/компании, внедряет инструменты вроде ML для прогнозирования спроса на кадры.",
                9: "Управляет аналитикой на уровне корпорации, формирует стратегию использования Big Data в HR, обеспечивает лидерство компании в сфере HR-аналитики.",
                10: "Управляет аналитикой на уровне корпорации, формирует стратегию использования Big Data в HR, обеспечивает лидерство компании в сфере HR-аналитики." // Дублирует 9 уровень (оригинальные данные)
            },

            "Формирование talent pool (кадрового резерва), наполнение и работа с базой кандидатов с учетом требований НЦЗПД, хантинг и долгий найм": {
                1: "Вносит кандидатов в базу (ФИО, контакт, краткая заметка), не нарушая требования по хранению данных.",
                2: "Периодически актуализирует статус кандидатов (доступен, в поиске, неактуален), запрашивает обновления резюме.",
                3: "Разбивает базу по сегментам (позиции, навыки), ведёт план регулярных касаний, чтобы поддерживать интерес перспективных кандидатов.",
                4: "Формирует кадровый резерв под ключевые вакансии, учитывает юридические аспекты (НЦЗПД), сроки хранения и обновления персональных данных.",
                5: "Разрабатывает стратегию формирования пула кандидатов: mailing-листы, участие в мероприятиях, поддержка отношений с «пассивными» соискателями.",
                6: "Настраивает комплексный хантинг: долгосрочную коммуникацию с топ-экспертами, ведёт несколько баз (например, для разных регионов), анализирует эффективность каждый квартал.",
                7: "Структурирует глобальный talent pool, создаёт программы pre-онбординга (формирует связь до выхода), считает ROI на формирование резерва и корректирует подход.",
                8: "Автоматизирует процессы (CRM, чат-боты, рассылки), проводит широкие кампании (прямой поиск дефицитных специалистов), использует аналитику для приоритизации связи.",
                9: "Влияет на рынок труда за счёт большого и качественного резерва, внедряет инновационные методы долгосрочного найма (собственные платформы, AI-сопоставление профилей).",
                10: "Влияет на рынок труда за счёт большого и качественного резерва, внедряет инновационные методы долгосрочного найма (собственные платформы, AI-сопоставление профилей)." // Дублирует 9 уровень (оригинальные данные)
            },
            "Взаимодействие с учебными заведениями и профессиональными сообществами (стажировки, практика, распределение, подрядчики и вендоры)": {
                4: "Участвует в карьерных днях и ярмарках вакансий, установив контакт с вузами/сообществами (приглашает на стажировки, отправляет материалы).",
                5: "Разрабатывает совместные стажировки и практики, поддерживает связь с профессиональными ассоциациями (покрывая потребности в узких специалистах).",
                6: "Курирует системное сотрудничество с вузами (курсы, мастер-классы), формирует контракты с подрядчиками, взаимодействует с профильными сообществами для приглашения на метапы.",
                7: "Формирует долгосрочные партнёрства (совместные образовательные программы, образовательные траектории), отвечает за масштабные встречи/форумы с вузами и комьюнити.",
                8: "Масштабирует взаимодействие на региональном и международном уровне, влияет на учебные планы, запускает крупные спонсорские и партнёрские проекты (конференции, конкурсы).",
                9: "Масштабирует взаимодействие на региональном и международном уровне, влияет на учебные планы, запускает крупные спонсорские и партнёрские проекты (конференции, конкурсы)."
            },

            "Разработка/исполнение roadmap и проведение анализа, координация руководства как на уровне предложений, так и на уровне отчетности (проведение PESTEL и SWOT-анализа)": {
                4: "Подготовляет пошаговый план найма (roadmap), применяет базовую SWOT-логику (сильные/слабые стороны, возможности, угрозы) для оптимизации процесса.",
                5: "Проводит анализ внешних факторов (PESTEL) – политических, экономических, социальных и др., корректирует план по результатам анализа.",
                6: "Составляет комплексные планы (roadmap + KPI), согласовывает их на уровне руководства, учитывая результаты проведённых SWOT/PESTEL.",
                7: "Организует реализацию стратегических планов, контролирует риски и вовремя предлагает изменения, опираясь на собранную аналитику.",
                8: "Управляет всем циклом Roadmap в HR, интегрирует результаты стратегических (PESTEL, SWOT) анализов в решения для топ-уровня (board/executives).",
                9: "Управляет всем циклом Roadmap в HR, интегрирует результаты стратегических (PESTEL, SWOT) анализов в решения для топ-уровня (board/executives)."
            },

            "Развитие и формирование HR-бренда и участие в конференциях, подготовка образовательных материалов, внедрение инновационных подходов и Agile-практик": {
                3: "Подключается к внутренним HR-активностям (корпоративные рассылки, блог), пишет короткие статьи/заметки, помогает при локальных событиях.",
                4: "Участвует во внешних мероприятиях (например, профессиональные конференции), помогает готовить материалы (презентации, кейсы), внедряет базовые Agile-практики в команде.",
                5: "Проводит доклады на внутренних и внешних конференциях, обучает коллег, продвигает HR-бренд в соцсетях и профессиональных площадках.",
                6: "Формирует стратегию продвижения HR-бренда, внедряет гибкие методологии (Scrum, Kanban) во внутренние процессы для повышения адаптивности.",
                7: "Влияет на имидж компании как топ-работодателя, организует участие в крупных конференциях, масштабирует инновационные подходы (design thinking, lean), поддерживает образовательные инициативы.",
                8: "Системно продвигает HR-бренд на рынке, интегрирует Agile-принципы во все HR-процессы, координирует работу флагманских проектов, делится экспертизой в масштабных публикациях.",
                9: "Системно продвигает HR-бренд на рынке, интегрирует Agile-принципы во все HR-процессы, координирует работу флагманских проектов, делится экспертизой в масштабных публикациях."
            },

            "Запуск пилотов и внедрение инноваций. Управление портфелем HR-проектов и внедрение новых решений/автоматизаций, в том числе AI-решений для оптимизации процессов. Использование HADI-циклов и метода SMART для качественной проработки продуктов и решений": {
                6: "Запускает пилотные инструменты найма (чат-бот, AI-фильтр резюме), проверяет гипотезы по HADI (цель, действие, данные, вывод), корректирует продукт.",
                7: "Управляет несколькими HR-проектами одновременно, задаёт SMART-цели (конкретные, измеримые и т.д.), оценивает результат и дорабатывает механики.",
                8: "Формирует инновационную экосистему HR (объединение AI-решений, автоматизация значимой части рекрутингового цикла), масштабирует пилоты на всю компанию, закрепляет их в регламентах.",
                9: "Формирует инновационную экосистему HR (объединение AI-решений, автоматизация значимой части рекрутингового цикла), масштабирует пилоты на всю компанию, закрепляет их в регламентах."
            },

            "Использование LinkedIn и X-Ray запросов для сорсинга специалистов, размещение вакансий, отслеживание активностей": {
                2: "Зарегистрирован в LinkedIn, умеет публиковать вакансии, ищет кандидатов по ключевым словам (basic search), отслеживает минимальную активность профилей.",
                3: "Понимает X-Ray-запросы, может составлять синтаксис для Google, Bing, делает выборку по нужным навыкам/локациям.",
                4: "Глубоко прорабатывает X-Ray (самостоятельные логические цепочки), почти не использует ручной перебор, сохраняет и актуализирует найденные профили.",
                5: "Сегментирует кандидатов (уровень, специализация), использует инструменты для массового сбора контактов, анализирует отдачу от этих инструментов.",
                6: "Обучает команду LinkedIn- и X-Ray-сорсингу, разрабатывает регламенты (как формировать запросы), выстраивает системную воронку из LinkedIn.",
                7: "Тиражирует стратегию поиска через LinkedIn и X-Ray на разные подразделения/регионы, настраивает плагины/интеграции для хранения и анализа профилей.",
                8: "Координирует международные поисковые активности (сорсинг в нескольких странах), осваивает инструменты LinkedIn Recruiter, внедряет инновационные методы (автоматизированные скрипты).",
                9: "Управляет корпоративными Premium-аккаунтами, строит глобальную сеть поиска, делится внутренними best practices на конференциях, обеспечивает конкурентное преимущество компании."
            },
